...
import { ManageOrderingListItemModel, ManageOrderingListRequestModel, ManageOrderingListResponse } from "../models/manage-ordering.model";

type ManageOrderingState = {
  initialized: boolean;

  isBusy: boolean;
  filterList: DftFilterItem[];
  posters: ManageOrderingListItemModel[];
...
const initialState: ManageOrderingState = {
  initialized: false,
  isBusy: false,
  filterList: [],
  posters: [],
...
const TAG = "[ManageOrderingStore]:";

export const ManageOrderingStore = signalStore(
  { providedIn: "root" },
  ...
  withMethods(({ _logger, _http, ...store }) => {
    ...
    const getPostersFromApi = () => {
      ...
      const requestBody: ManageOrderingListRequestModel = {
        maxResults: 50,
      };
      ...
      appliedFilters.forEach((flt) => {
        ...
        switch (flt.name) {
          case MANAGE_ORDERING_FILTERS.category.name:
            requestBody.categoryId = flt.value as string;
            break;
          case MANAGE_ORDERING_FILTERS.languages.name:
            requestBody.languageIds = Array.isArray(flt.value) ? flt.value : [flt.value].filter(Boolean);
            break;
          case MANAGE_ORDERING_FILTERS.posterType.name:
            requestBody.posterType = flt.value as string;
            break;
          case MANAGE_ORDERING_FILTERS.status.name:
            requestBody.active = flt.value === "ACTIVE";
            break;
          case MANAGE_ORDERING_FILTERS.publishedAt.name:
            ...
            break;
          case MANAGE_ORDERING_FILTERS.createdAt.name:
            ...
            break;
          case MANAGE_ORDERING_FILTERS.party.name:
            requestBody.partyId = flt.value as string;
            break;
          case MANAGE_ORDERING_FILTERS.partyState.name:
            requestBody.stateId = flt.value as string;
            break;
        }
      });

      return _http.post<ManageOrderingListResponse>("/posters/paginated", requestBody).pipe(
        catchError((error) => {
  ...
  withHooks(({ _logger, _http, ...store }) => {
    ...
    const getFilterStateOptions = (optFlt: DftFilterOptionFilter): Observable<DftFilterOption[]> => {
      ...
      if (!optFlt.dependencyContext || !optFlt.dependencyContext[MANAGE_ORDERING_FILTERS.party.name]) {
      ...
      const partyId = optFlt.dependencyContext[MANAGE_ORDERING_FILTERS.party.name];
    ...
    const loadFilters = () => {
      const filters = Object.values(MANAGE_ORDERING_FILTERS).map((f) => {
        const flt = { ...f };
        switch (f.name) {
          case MANAGE_ORDERING_FILTERS.category.name:
            flt.options = [];
